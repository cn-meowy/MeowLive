...
const __bili_loginGlobalKey = "__lp_plugin_bilibili_1_0_11_login";

async function _bili_request(request, authMode) {
  return await _bili_loginHelper().request(request, authMode);
}
...
function _bili_loginHelper() {
  const helper = globalThis[__bili_loginGlobalKey];
  if (!helper) {
    _bili_throw("UNSUPPORTED", "bilibili login helper is unavailable", {});
  }
  return helper;
}
...
async function _bili_getLoginUID(authMode) {
  return await _bili_loginHelper().getLoginUID(authMode);
}
        ...
        for (const codecInfo of codecList) {
          ...
          for (const urlInfo of urlInfoList) {
            ...
            hostQualityMap[host].push({
              ...
              requestContext: {
                qn: String(qualityMeta.qn || 1500),
                streamType: streamInfo.protocol_name === "http_hls" ? "hls" : "flv"
              },
              playbackHints: {
                streamFormat: streamInfo.protocol_name === "http_hls" ? "hlsLive" : "flv",
                requiresCustomSegmentLoader: streamInfo.protocol_name === "http_hls"
              }
            });
          }
        }
...
globalThis.LiveParsePlugin = {
  ...
  async setCookie(payload) {
    return await _bili_loginHelper().setCookie(payload);
  },

  async clearCookie() {
    return await _bili_loginHelper().clearCookie();
  },

  async setCredential(payload) {
    return await _bili_loginHelper().setCredential(payload);
  },

  async clearCredential() {
    return await _bili_loginHelper().clearCredential();
  },

  async getCredentialStatus(payload) {
    return await _bili_loginHelper().getCredentialStatus(payload);
  },

  async validateCredential(payload) {
    return await _bili_loginHelper().validateCredential(payload);
  },
