...
function _cc_buildSockJSURL(rawURL) {
  ...
  if (/\/websocket$/i.test(parts.path || "")) return `wss://${parts.authority}${parts.path}${parts.query}`;
  ...
  return `wss://${parts.authority}${basePath}/${String(Math.floor(Math.random() * 1000)).padStart(3, "0")}/${Math.random().toString(16).slice(2, 10).padEnd(8, "0")}/websocket${parts.query}`;
}
...
const __lp_cc_sharedGlobalKey = "__lp_plugin_cc_1_0_4_shared";
