...
const __soop_loginGlobalKey = "__lp_plugin_soop_1_0_10_login";
...
function _soop_loginHelper() {
  var helper = globalThis[__soop_loginGlobalKey];
  if (!helper) {
    _soop_throw("UNSUPPORTED", "soop login helper is unavailable", {});
  }
  return helper;
}
...
function _soop_isNumericId(val) {
  return /^\d+$/.test(_soop_str(val));
}

function _soop_liveCoverURL(broadNo) {
  var id = _soop_str(broadNo);
  if (!_soop_isNumericId(id)) return "";
  return "https://liveimg.sooplive.co.kr/m/" + encodeURIComponent(id);
}

function _soop_profileImageURL(userId) {
  var id = _soop_str(userId);
  if (!/^[a-zA-Z0-9_]{2,20}$/.test(id)) return "";
  return (
    "https://stimg.sooplive.co.kr/LOGO/" +
    encodeURIComponent(id.slice(0, 2).toLowerCase()) +
    "/" +
...
function _soop_isAdultRestrictedChannel(channel) {
  return Number(channel && channel.RESULT) === -6;
}
...
globalThis.LiveParsePlugin = {
  ...
  async setCookie(payload) {
    return await _soop_loginHelper().setCookie(payload);
  },

  async clearCookie() {
    return await _soop_loginHelper().clearCookie();
  },

  async setCredential(payload) {
    return await _soop_loginHelper().setCredential(payload);
  },

  async clearCredential() {
    return await _soop_loginHelper().clearCredential();
  },

  async getCredentialStatus(payload) {
    return await _soop_loginHelper().getCredentialStatus(payload);
  },

  async validateCredential(payload) {
    return await _soop_loginHelper().validateCredential(payload);
  },
  ...
  async getRoomDetail(payload) {
    ...
    var bjId = userId || roomId;
    var restrictedLive = false;
    var restrictedTitle = "";
    var requestedBroadNo = _soop_isNumericId(roomId) ? roomId : "";
    if (!bjId)
      _soop_throw("INVALID_ARGS", "roomId or userId is required", {
        field: "roomId",
      });
    ...
    try {
      ...
      if (Number(channel.RESULT) === 1 && channel.BNO) {
        return {
          ...
          roomCover:
            _soop_liveCoverURL(channel.BNO),
          userHeadImg: _soop_profileImageURL(channel.BJID || bjId),
          liveState: "1",
      ...
      if (_soop_isAdultRestrictedChannel(channel)) {
        restrictedLive = true;
        restrictedTitle = _soop_str(channel.TITLE);
      }
    } catch (e) {
      // ignore
    }
    ...
    try {
      ...
      if (Number(latestChannel.RESULT) === 1 && latestChannel.BNO) {
        return {
          ...
          roomCover:
            _soop_liveCoverURL(latestChannel.BNO),
          userHeadImg: _soop_profileImageURL(latestChannel.BJID || bjId),
          liveState: "1",
      ...
      if (_soop_isAdultRestrictedChannel(latestChannel)) {
        restrictedLive = true;
        restrictedTitle = restrictedTitle || _soop_str(latestChannel.TITLE);
      }
    } catch (e) {
      // ignore
    }
    ...
    if (stationData) {
      var fallbackUserId = _soop_str(stationData.user_id || bjId);
      var fallbackRoomId =
        requestedBroadNo ||
        _soop_str(stationData.broad_no || stationData.station_no || bjId);
      return {
        userName: _soop_str(stationData.user_nick || stationData.station_name),
        roomTitle: _soop_str(
          restrictedTitle || stationData.station_title || stationData.station_name || ""
        ),
        roomCover: _soop_liveCoverURL(fallbackRoomId),
        userHeadImg: _soop_str(stationData.user_profile_img || "") || _soop_profileImageURL(fallbackUserId),
        // broad_start 可能是历史开播时间，不能作为在线状态判定依据
        liveState: restrictedLive && requestedBroadNo ? "1" : "0",
