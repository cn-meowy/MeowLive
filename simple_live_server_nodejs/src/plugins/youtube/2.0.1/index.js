...
function _yt_extractInnertubeContext(html) {
  var source = _yt_str(html);
  var marker = "ytcfg.set(";
  var offset = 0;

  while (offset < source.length) {
    var markerIndex = source.indexOf(marker, offset);
    if (markerIndex < 0) break;
    var config = _yt_extractJSONAfterMarker(source.slice(markerIndex), marker);
    if (config && config.INNERTUBE_CONTEXT && typeof config.INNERTUBE_CONTEXT === "object") {
...
function _yt_buildPlayback(videoId, manifestURL, variants, options) {
  ...
  if (Array.isArray(variants) && variants.length > 0) {
    for (var i = 0; i < variants.length; i += 1) {
      ...
      qualitys.push({
        ...
        requestContext: { qn: String(qn), streamType: "hls" },
        playbackHints: {
          streamFormat: "hlsVod",
  ...
  if (ordered.length === 0) {
    ordered.push({
      ...
      userAgent: playbackProfile.userAgent,
      headers: playbackProfile.headers,
      requestContext: { qn: "0", streamType: "hls" },
      playbackHints: {
        streamFormat: "hlsVod",
        requiresCustomSegmentLoader: true
      }
...
globalThis[__yt_sharedGlobalKey] = {
  ...
  extractInnertubeContext: _yt_extractInnertubeContext,
